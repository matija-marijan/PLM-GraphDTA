...
launch_job() {
    local gpu="$1"; shift
    local args=( "$@" )
    # build tidy log filename based on model/dataset/seed if available
    local mdl=""
    local dset=""
    local seed=""
    for (( i=0; i<${#args[@]}; i++ )); do
    ...
    local logfile="logs/${mdl:-job}_${dset:-dataset}_seed_${seed:-unk}_$$.log"
...
log "All testing jobs completed!"