...
class PLM_Vnoc_GINConvNet(torch.nn.Module):
    def __init__(self, n_output=1,num_features_xd=78, num_features_xt=25,
        ...
        self.bn_fc_xt_layers = nn.ModuleList()
        prev_dim = self.embed_dim
        for out_dim in self.plm_layers:
            self.fc_xt_layers.append(nn.Linear(prev_dim, out_dim))
            self.bn_fc_xt_layers.append(nn.BatchNorm1d(out_dim))
            prev_dim = out_dim
    ...
    def forward(self, data):
        ...
        xt_plm = target_embedding
        for fc, bn in zip(self.fc_xt_layers, self.bn_fc_xt_layers):
            xt_plm = fc(xt_plm)
            xt_plm = bn(xt_plm)
            xt_plm = self.relu(xt_plm)

        xt = torch.cat((xt, xt_plm), dim=1)
